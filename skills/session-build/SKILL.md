...
- **A gate that did not finish did not decide.** Before believing a rejection, check that the gate *ran to completion*. One killed by a timeout — its own, or the hook runner's — rejected nothing; it ran out of time, which is a fact about duration, not about your code. From the inside the two are indistinguishable, and the instinct they provoke is either `--no-verify` or rewriting code that was never wrong. Observed: a static-analysis gate timing out because a deploy-set collision had inflated its input until it was type-checking three functions from another branch.
- **One root cause wears several costumes.** That same collision surfaced as a collision-gate failure *and* as a timeout in an unrelated gate, and the fork treated them as two problems. When two gates go red in the same window, ask whether one cause explains both **before** fixing either — two independent fixes for one cause usually means one of them is damage.
...
| "The gate rejected it, so the code is wrong" | Check it finished. A gate killed by a timeout decided nothing, and looks identical to one that decided against you. |
| "Two gates are red, so there are two problems" | Usually one cause in two costumes. Fixing both independently means one fix is damage. |
