...
- **For a shared target, the mitigation is not a lock — it is re-reading at the moment of writing.** The default branch is the one surface no worktree isolates, and the contention there is usually not with your own forks (they never touch it) but with **another session on the machine**. Two things sufficed in a five-fork run, and the second is the one that pays: run the docs phase **strictly last**, after every fork has closed, so it never races your own work; and **re-read every target file as you write it**, instead of trusting the survey you did at Step 2. That re-read immediately found **three of four documents already corrected** by the other session — the pending item was itself the stale thing — and caught an item that would have deleted 27 of a view's 72 lines, visible only because the neighbour had measured and rewritten it in the meantime. Same rule as topology, applied to files instead of branches.
...
**And measure it at the moment you write the instruction, because the answer moves.** In that same run, a later push shifted one branch six commits ahead of the other two: the collapse went from *any of the three delivers everything* to *only this one does*, within the hour. A close-out naming the wrong branch after such a shift instructs the user to merge something incomplete. Topology is a timestamp too — re-check if any fork has pushed since you last looked.
