...
### Every measurement is a reading, not a property

**The single rule this skill has learned the most ways to break.** Anything you measure during a run is true *of the instant you measured it*, and nothing tells you when it stops being true. Concurrent forks, concurrent sessions and the user all keep moving. It shows up as at least six different bugs, each of which cost a real run:

| What you measured | How it goes stale |
|---|---|
| A gate's exit code | A peer commits; an ancestry gate flips back to red, silently → *Step 5, Green is a timestamp* |
| Branch topology | A late push moves a head; the branch you named no longer carries the work → *Step 6* |
| "These branches are independent" | Anything merging anywhere can falsify it — it decays fastest of all → *Step 6* |
| A derived count | Remote-only migrations went 9 → 16 in hours → below |
| A file you plan to edit | Another session already rewrote it, or already fixed the thing you were going to fix → below |
| A fork's report | It was true when written and the fork has since pushed → *Step 5* |

So the discipline is always the same: **measure immediately before the action the measurement authorises, and record the instant beside the value.** A reading with no timestamp is an instruction with an expiry date that does not say what it is.
...
- **Never carry a derived number forward — re-derive it at the moment of use** (*Every measurement is a reading*, above). Observed: the count of remote migrations without a local file went **9 → 13 → 14 → 15 → 16** within hours, because peers kept applying.
...
2. **Dependency graph — and expect it to be boring.** For each ordered pair, classify in plain prose. **This classification is repair for a bad cut, not the main event:** a run whose specs were cut along file surface classified ten pairs and wrote one word, *independent*, for all of them, because that cut produces independence by construction. If most pairs are coming out entangled or ordered, stop classifying and go back to the decomposition — you are describing a problem you could have avoided. The two things that mattered in that run are both outside the taxonomy: **the acyclicity check**, and **lock serialisation, which is orthogonal to dependency** — three mutually independent specs still queued for the same migration mutex. **Every tier keeps its own branch and its own worktree — two sessions never share a working directory.** A dependency is temporal, not spatial: a fork that needs another's code cannot implement before that code exists, so sharing a directory buys parallelism the dependency itself forbids, while importing `index.lock` contention, verification that reads a neighbour's half-written files, and working-tree gates that fail on someone else's breakage.
   - **Independent** — no shared surface. Fully parallel. **Verify this transitively before believing it**, because it is the classification that fails silently and the one whose failure costs most: two specs that name no file in common are still a single **deploy unit** if one modifies a bundled shared module and the other touches *any consumer of it — including deleting that consumer*. A bundled module is copied into every consumer at deploy time, so the deploy set is computed through the import graph, not from the file list either spec wrote. Misfiled as `Independent`, this pair deadlocks late (see *The merge-direction deadlock*, Step 5). Treat it as `Ordered` at minimum, with the deploy ordering settled here rather than discovered later.
   - **Ordered** — B needs *something* of A's before it can finish: its interface decision, or its code for some of B's tasks. Same answer either way, which is why this is one tier and not two: B branches from the default branch, **orders its plan so every dependency-free task runs first**, and puts the rest behind a single marked merge point. The orchestrator relays the interface decision when there is one, and sends `MERGE <A-branch> BEFORE <task>` when A reports `PUSHED`. B builds while A builds instead of idling.
   - **Total** — B imports A's code wholesale, or B's migration assumes A's schema, and almost nothing in B stands alone. **B's branch is created from A's branch**, after A reports `PUSHED` — so integration costs nothing, there is no merge to save. B still plans and codex-reviews in parallel, from the default branch, while it waits.
      **Decompose by where the diff lands, not by theme — this is what decides whether `N` scales.** Cut along topics and the specs' manifests overlap, so the manifest intersection becomes genuinely `N²` work and every pair needs arbitration. Cut along **file and deploy surface** and the manifests come out nearly disjoint by construction, so the intersection is `N` membership checks that take seconds each. Measured: a five-spec run produced 10 pairs and dismissed all of them in seconds, precisely because five candidate items that converged on two files had been folded into **one** spec instead of five. The corollary from a different run points the same way: when the import graph joins two specs they are one spec, not two. **Splitting what lands in the same place buys a merge order, not parallelism.**
...
For a **total** dependency, the base ref is the dependency's branch instead — and that worktree is created only after the dependency reports `PUSHED`. An **ordered** dependency still branches from the default branch and starts immediately; it takes the dependency later, at its plan's merge point.
...
This is not a path-normalisation bug — `git worktree list --porcelain` reports the exact path and `pwd -P` inside it matches byte-for-byte.

**The refusal is specific to forks, and an earlier version of this skill wrongly generalised it to everyone.** Three forks across two runs reproduced it; an orchestrator, separately, **entered five worktrees in sequence with no refusal at all** and returned to the main checkout with `ExitWorktree`. So: a fork cannot enter, a plain session can. Treat the fork case as reproduced, and the orchestrator case as reported by one run and not re-tested here. The practical consequence is in Step 6 — the close-out must **not** tell the user that a fresh session is the only way to run `/session-end`.
...
**N ≥ 2:** the orchestrator forks itself once per spec — all forks dispatched **in a single message** so they start concurrently — using the contract in [dispatch-prompts.md](dispatch-prompts.md) § Fork implementer. Forks with an unmet **total** dependency are launched in `HOLD` state: they plan and codex-review, then stop and wait for a `GO`. Forks with an **ordered** dependency are launched normally — their plan already orders the blocked tasks last.
...
**The `agentId` from the spawn result is the address. Record it at spawn or lose it.** `description` is not a naming lever — eight forks across two runs were dispatched with distinct slugs and **every one listed as a bare handle**, so treat it as display text only and never as something you can address later. A fork told to `COORDINATE WITH <slug>` gets `No agent named '<slug>' is reachable`; the correct response, refusing to guess a handle, costs the orchestrator a relay round-trip. So: write `spec slug → agentId` into the ledger the moment each fork spawns, and **send that whole map to every fork in its dispatch prompt** — without it, `COORDINATE WITH` is fiction. Pass the slug as `description` anyway, for the run's display:
...
**A replacement design must clear the same bar as the design it replaces.** The single most valuable rule in this file, and it nearly cost a production outage.
...
The trap is structural, not careless. **The replacement arrives welded to the refutation of the original**, the refutation is genuinely convincing, and so the new design inherits the credibility of the argument that killed the old one. That is the moment of least scepticism in the whole exchange — and it is exactly when scepticism is cheapest, because nothing has been written yet. So: **whatever evidence you demanded before rejecting, demand again before endorsing.** Say it in the same message that concedes the point.
...
- **`LOCK verify` — one fork at a time through lint, typecheck, build, suite and push.** A third first-class lock, on the same protocol as the other two. It exists because **the database and the deploy runtime are shared and serialised, while the CPU is shared and was not** — and because the two contentions fail in opposite ways:
  ...
  Measured across two independent runs on one machine: load average **80.61**, peaking at **98.88**, on ~10 cores, with up to 40 test/lint processes from eight forks; a `git push` stuck **43 minutes** in its pre-push hook; a lint-plus-typecheck that **did not finish in 600 s** on a branch that touched no application code. The lock was introduced mid-run and load fell from 52.89 to 47.11 immediately. Grant it as you grant the others: one holder, `GO` to exactly one, wait for release.

  **Of everything that changes when `N` grows, this is the only one that scales non-linearly**, and that is what makes it qualitative rather than merely worse: below a threshold, more forks cost time; above it, **verification stops producing a result at all**. Three forks fit the machine. Five plus two unrelated sessions did not, and that is the moment the lock stopped being an optimisation.
...
**And measure it at the moment you write the instruction, because the answer moves.** In that same run, a later push shifted one branch six commits ahead of the other two: the collapse went from *any of the three delivers everything* to *only this one does*, within the hour. A close-out naming the wrong branch after such a shift instructs the user to merge something incomplete. Re-check if any fork has pushed since you last looked (*Every measurement is a reading*).

**And the two claims do not decay at the same rate.** *"These are independent"* ages far faster than *"this one depends on that one"*, because independence is a claim about **the whole world** and dependence is a claim about **two objects**. Anything merging anywhere can falsify the first; only those two branches can falsify the second. Observed: a close-out stating "no mandatory merge order, all five are independent" — verified against the real commits when written — was false in under an hour, when an unrelated branch merged, touched a file one of the five also touched, and turned its pull request into a conflict that had to be resolved by giving up the file and shrinking that branch's scope. **So state independence with its timestamp and a shorter shelf life than any dependency you print beside it.** This skill does not open PRs and does not merge.
...
**The orchestrator can do it, and this is the cheapest path.** The `EnterWorktree` refusal in Step 3 binds **forks**, not a plain session: one orchestrator entered all five of its worktrees in sequence, closed each branch, and returned to the main checkout with `ExitWorktree`. So offer that first — it already holds the whole picture and every fork's close-out, and it needs no context transfer.

**Two things about `/session-end` break in that mode and it must be told about them.** Its Step 0 stops when the current branch is the default one — which is exactly the orchestrator's correct starting state, so read that as *"enter each worktree"*, not *"abort"*. And a session that is inside a worktree has **compound bash refused**: no `cd X && …`, no redirects, no `for`, and no `-C` pointing at a different worktree. Every command becomes simple and one per call, which changes how each step is written and is announced nowhere.

**A fresh session launched from the worktree remains a valid option**, not a requirement — `cd .claude/worktrees/<slug>-<date>` and start there. Everything that step needs from the run is in the ledger; each fork's `PARKED` and `CUT` entries exist precisely so a session that did not build the branch can still write honest pendings.
